...
### 集成化 B4X 布局视图
- **可视化布局编辑器**：在可视化设计画布中打开 `.bjl`（B4J）和 `.bal`（B4A）文件，支持拖拽、调整大小、选择、缩放和平移。
...
- **新建布局文件**：通过 `B4X: New Layout File...` 命令创建新的 `.bjl`/`.bal` 布局文件。自动发现工作区中的 B4A/B4J 项目。

### AI Agent CLI 工具
- **`b4x-cli`**：自动在集成终端中可用的 CLI 工具，用于在二进制格式（`.bjl`/`.bal`）和 JSON 格式之间转换布局文件——专为 AI Agent 使用而设计。
